...
let LeadsHunter, BusinessFinder, EmailSender, LandingPageBuilder, StripeManager, Anthropic;
let INDUSTRIES, EMAIL_TEMPLATES;
let leadsHunter, businessFinder, emailSender, landingBuilder, claude, stripeManager;

async function initializeModules() {
  try {
    const mod1 = await import("./business-finder.js");
    BusinessFinder = mod1.default;
  } catch (e) {
    console.warn("⚠️ BusinessFinder import failed:", e.message);
  }

  try {
    const mod2 = await import("./leads-hunter.js");
    LeadsHunter = mod2.default;
  } catch (e) {
    console.warn("⚠️ LeadsHunter import failed:", e.message);
  }

  try {
    const mod3 = await import("./email-sender.js");
    EmailSender = mod3.default;
  } catch (e) {
    console.warn("⚠️ EmailSender import failed:", e.message);
  }

  try {
    const mod4 = await import("./landing-builder.js");
    LandingPageBuilder = mod4.default;
  } catch (e) {
    console.warn("⚠️ LandingPageBuilder import failed:", e.message);
  }

  try {
    const mod5 = await import("./stripe-manager.js");
    StripeManager = mod5.default;
  } catch (e) {
    console.warn("⚠️ StripeManager import failed:", e.message);
  }

  try {
    const mod6 = await import("./config.js");
    INDUSTRIES = mod6.INDUSTRIES || [];
    EMAIL_TEMPLATES = mod6.EMAIL_TEMPLATES || [];
    SERVER_CONFIG = mod6.SERVER_CONFIG || { DATA_DIR: "./data", UPLOADS_DIR: "./uploads", PORT: 3000 };
  } catch (e) {
  ...
  try {
    const mod7 = await import("@anthropic-ai/sdk");
    Anthropic = mod7.default;
  } catch (e) {
  ...
  if (BusinessFinder) {
    try {
      ...
      console.warn("⚠️ BusinessFinder initialization failed:", e.message);
    }
  }
...
async function handleAPI(req, res, url, method) {
  ...
  // 🏢 BUSINESS SEARCH (Geographic + Category-based)
  if (endpoint === "businesses") {
    if (action === "search" && method === "POST" && businessFinder) {
      const body = await parseBody(req);
      try {
        const results = await businessFinder.searchBusinesses(
          body.query || body.keywords || "",
          body.location || "Internacional",
          body.type || "business"
        );
    ...
    if (action === "list" && method === "GET" && businessFinder) {
      return json(res, { results: businessFinder.results || [], count: (businessFinder.results || []).length });
    }

    if (action === "export" && method === "GET" && businessFinder) {
      const format = new URL(req.url, `http://${req.headers.host}`).searchParams.get("format") || "csv";
      if (format === "json") {
        return json(res, { success: true, data: JSON.parse(businessFinder.exportToJSON()) });
      } else {
        res.writeHead(200, { "Content-Type": "text/csv" });
        return res.end(businessFinder.exportToCSV() || "");
      }
    }
  }

  // 🎯 LEADS HUNTING (Email-based search)
  if (endpoint === "leads") {
